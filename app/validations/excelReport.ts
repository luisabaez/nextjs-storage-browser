...
export interface ReportFile {
  ...
  populationCount?: number;    // true population when only the sample was loaded (server-sampled); else population.length
}
...
function relationshipsSheet(wb: ExcelJS.Workbook, files: ReportFile[], meta: SampleMeta, used: Set<string>, hasPopulation: boolean): void {
  ...
  for (const f of files) {
    ...
    const row = ws.addRow([f.label, role, f.populationCount ?? f.population.length, f.sample.length, link, cv]);
...
function sizingSheet(wb: ExcelJS.Workbook, meta: SampleMeta, files: ReportFile[], used: Set<string>): void {
  ...
  const rows: unknown[][] = [
    ...
    ...files.map(f => [f.label, f.populationCount ?? f.population.length, f.sample.length]),
  ];
  rows.forEach((r, i) => {
...
export function mergeResultToReportFiles(r: MergeResult, selectedIndices: number[], parentLabel?: string): ReportFile[] {
  ...
  const files: ReportFile[] = [{
    ...
    role: 'parent',
    populationCount: r.parentPopulation,
  }];
