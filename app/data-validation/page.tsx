...
function DataValidationPage() {
  ...
  const loadSummary = useCallback(async (prog: string, src: string) => {
    ...
    try {
      ...
      const rep = await (await fetch(`${LAMBDA_URL}?action=val_reports&mock=${PLAN_MOCK}&program=${encodeURIComponent(prog)}`
  ...
  const openReport = async (key: string) => {
    try {
      ...
      setError(`Could not open report: ${(e as Error).message}`);
    }
  };
    ...
    <div className="dv-page">
      ...
      {runResult && (
        <section className={`dv-card ${runResult.ok ? 'dv-card-ok' : 'dv-card-err'}`}>
          ...
          {runResult.report_key && (
            <p>
              Client report: <button className="dv-link" onClick={() => openReport(runResult.report_key!)}>{runResult.report_name}</button>
      ...
      {program && reports.length > 0 && (
        <section className="dv-card">
          ...
          <p className="dv-muted small">Excel workbooks (Data + Pivot) generated by each run, stored under <code>DataValidation/Reports/{PLAN_MOCK}/</code> in the file browser. Send the latest one per file to the client.</p>
          <div className="dv-scroll">
            <table className="dv-table">
              ...
              <tbody>
                {reports.slice(0, 40).map(r => (
                  <tr key={r.key}>
                    ...
                    <td className="mono">{r.source}</td>
                    <td><button className="dv-link" onClick={() => openReport(r.key)}>{r.name}</button></td>
