...
export const handler: PreSignUpTriggerHandler = async (event) => {
  ...
  console.log(`New user attempting to sign up: ${userEmail}`);

  // Send notification email to admin
  try {
    const approveUrl = `https://console.aws.amazon.com/cognito/v2/idp/user-pools/${userPoolId}/users?region=${region}`;
    ...
    const emailParams = {
      ...
      Message: {
        ...
        Body: {
          ...
          Text: {
            Data: `New User Registration Request\n\nEmail: ${userEmail}\nUsername: ${userName}\nTime: ${new Date().toISOString()}\n\nTo approve: aws cognito-idp admin-confirm-sign-up --user-pool-id ${userPoolId} --username ${userEmail}`,
            Charset: "UTF-8",
          },
        },
      },
    ...
    // Continue with sign-up even if email fails - user will still be unconfirmed
  }

  // CRITICAL: Do NOT auto-confirm the user
  // This keeps them in UNCONFIRMED status until admin approves
  event.response.autoConfirmUser = false;
  event.response.autoVerifyEmail = false;
  event.response.autoVerifyPhone = false;

  console.log("User will remain UNCONFIRMED until admin approves");
  console.log("=== PRE SIGN UP TRIGGER COMPLETE ===");
